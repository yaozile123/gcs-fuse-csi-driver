...
func (t *gcsFuseCSIGCSFuseIntegrationFileCacheParallelDownloadsTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	gcsfuseIntegrationFileCacheTestNew := func(testName string, readOnly bool, fileCacheCapacity string, mountOptions ...string) {
		...
		commandArgs := []string{
			fmt.Sprintf("export GO_VERSION=$(%v)", gcsfuseGoVersionCommand),
