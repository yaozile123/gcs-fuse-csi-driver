...
func cloneVolumeResource(res *storageframework.VolumeResource) *storageframework.VolumeResource {
	if res == nil {
		return nil
	}

	cloned := &storageframework.VolumeResource{
		Pvc: res.Pvc,
	}

	if res.VolSource != nil {
		volSourceCopy := *res.VolSource
		if res.VolSource.CSI != nil {
			csiCopy := *res.VolSource.CSI
			csiCopy.VolumeAttributes = make(map[string]string)
			for k, v := range res.VolSource.CSI.VolumeAttributes {
				csiCopy.VolumeAttributes[k] = v
			}
			volSourceCopy.CSI = &csiCopy
		}
		cloned.VolSource = &volSourceCopy
	}
...
func generateTestCommand(opts TestCommandConfig) string {
	goTestCmd := fmt.Sprintf("GODEBUG=asyncpreemptoff=1 go test ./%v/... -p 1 --integrationTest -v --config-file=../test_config.yaml", opts.TestPkg)
	if opts.TestName != "" {
		goTestCmd = fmt.Sprintf("GODEBUG=asyncpreemptoff=1 go test ./%v/... -p 1 --integrationTest -v -run ^%v$ --config-file=../test_config.yaml", opts.TestPkg, opts.TestName)
	}

	commandArgs := []string{
		fmt.Sprintf(gcsfuseGoEnvSetupFormat, opts.GoEnvSetupCmd),
		fmt.Sprintf("export MOUNTED_DIR=%v", opts.MountPath),
	}
	...
	if opts.SecondaryMountPath != "" {
		commandArgs = append(commandArgs, fmt.Sprintf("export MOUNTED_DIR_SECONDARY=%v", opts.SecondaryMountPath))
	}

	commandArgs = append(commandArgs,
		fmt.Sprintf("export BUCKET_NAME=%v", opts.BucketName),
		fmt.Sprintf("export ONLY_DIR=%v", opts.OnlyDir),
...
func (t *gcsFuseCSIGCSFuseIntegrationTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	gcsfuseIntegrationTest := func(testName string, readOnly bool, mountOptions ...string) {
		...
		installGcsfuseIntegrationDependencies(tPod, f, gcsfuseTestBranch, true)
	...
	gcsfuseIntegrationTestNew := func(testPkg string, testName string, config utils.ParsedConfig, secondaryConfig *utils.ParsedConfig) {
		...
		// Generate a unique profile label for cloud_profiler tests dynamically.
		// It is injected into vars here to ensure the exact same label is expanded in mountOptions
		// and later exported as environment variable to the test pod.
		if testPkg == testNameCloudProfiler || testName == testNameCloudProfiler {
		...
		if secondaryConfig != nil {
			...
			clonedRes := cloneVolumeResource(l.volumeResource)
			tPod.SetupVolume(clonedRes, volumeName2, mountPath2, secondaryConfig.ReadOnly, secondaryConfig.MountOptions...)
		}
	...
	generateDynamicTests := func(configVersion string) {
		...
		for pkgName, pkgList := range utils.LoadedTestPackages {
			if pkgName != "rapid_appends" && pkgName != "cloud_profiler" {
				continue
			}
			...
			for _, config := range pkg.Configs {
				...
				for i, flagStr := range config.Flags {
					// Standard workarounds from other suites
					...
					ginkgo.It(fmt.Sprintf("should succeed in %s with flags %s", fullTestName, flagStr), func() {
						...
						if pkgName == testNameCloudProfiler {
							cleanupIAM, err := utils.AddCloudProfilerBindings(ctx, f.Namespace.Name)
							if err != nil {
								framework.Logf("Failed while prepping cloud profiler iam bindings for tests: %v", err)
							} else {
								ginkgo.DeferCleanup(cleanupIAM)
							}
						}
						...
						if pkgName == "monitoring" && testName != "" {
							...
							for i, opt := range parsedFlags.MountOptions {
								if strings.HasPrefix(opt, "logging:file-path:") {
									parsedFlags.MountOptions[i] = "logging:file-path:" + logFileName
									break
								}
							}
						}
						...
						gcsfuseIntegrationTestNew(pkgName, testName, parsedFlags, secondaryParsedFlags)
					})
				}
			}
		}
...
func installGcsfuseIntegrationDependencies(tPod *specs.TestPod, f *framework.Framework, gcsfuseTestBranch string, installGcloud bool) {
